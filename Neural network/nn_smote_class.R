...
somte_methods <- SMOTE(prep_train[c(19:23,26:63)], perc_min = 50, k = 2)
...
rd_m<-round_df(somte_methods[c(1:42)],0)
joined <-cbind(rd_m, somte_methods[c(43)])
...
metrics_train <- data.matrix(joined[c(1:42)])
metrics_test <- data.matrix(prep_test[c(19:23,26:62)])
...
compute <- function(activation_f, n_layers, n_units,n_epochs){
  ...
  model %>% 
    layer_dense(units = n_units, activation = activation_f, input_shape = c(42))
  for(i in 1:n_layers){
