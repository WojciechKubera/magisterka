...
somte_methods <- SMOTE(prep_train[c(21:44,47)], perc_min = 50, k = 2)
...
rd_m<-round_df(somte_methods[c(1:24)],0)
joined <-cbind(rd_m, somte_methods[c(25)])
...
metrics_train <- data.matrix(joined[c(1:24)])
metrics_test <- data.matrix(prep_test[c(21:44)])
...
compute <- function(activation_f, n_layers, n_units,n_epochs){
  ...
  Sys.sleep(1)
  jpeg(sprintf("%s_softmax%s_layers_%d_units_%d_epochs_%d.jpeg",recognized_smell,activation_f, n_layers, n_units,n_epochs), width = 500, height = 500)
  plt<-plot(history)
  print(plt)
  print("render")
  print(sprintf("%s_softmax%s_layers_%d_units_%d_epochs_%d.jpeg",recognized_smell,activation_f, n_layers, n_units,n_epochs))
  Sys.sleep(1)
  dev.off()
  ...
  capture.output(cm, file = sprintf("%s_softmax%s_layesr_%d_units_%d_epochs_%d.csv",recognized_smell,activation_f, n_layers, n_units, n_epochs))
  
  jpeg(sprintf("%s_softmax%s_layesr_%d_units_%d_epochs_%dcfm.jpeg",recognized_smell ,activation_f, n_layers, n_units, n_epochs), width = 500, height = 500)
