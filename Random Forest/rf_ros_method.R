...
minority_methods <- (data.frame(methods[c(21:45, 47)]))
...
metrics_all <- data.matrix(methods_all[21:44])
# metrics <- normalize(data.matrix(methods[21:45]))

metrics_train <- data.matrix(prep_train[21:44])
metrics_test <- data.matrix(prep_test[21:44])
...
rf <- randomForest(is_smell~.,data=prep_train[c(21:44, 47)], ntree=100)

print(rf)
floor(sqrt(ncol(prep_train[c(21:44)]) - 1))

mtry <- tuneRF(prep_train[c(21:44)],prep_train$is_smell, ntreeTry=100,
               stepFactor=1.5,improve=0.01, trace=TRUE, plot=TRUE)
best.m <- mtry[mtry[, 2] == min(mtry[, 2]), 1]
print(mtry)
print(best.m)

rf <-randomForest(is_smell~.,data=prep_train[c(21:44,47)], mtry=best.m, importance=TRUE,ntree=100)
print(rf)
...
prep_test$prediction <- predict(rf, newdata=prep_test[c(21:44)])
